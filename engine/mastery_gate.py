...
class MasteryGate:
    def __init__(self, prd_areas=None, diversification_threshold=0.5):
        self.prd_areas = prd_areas if prd_areas is not None else [
            'arch', 'setup', 'code', 'test', 'security', 'scaling', 'ux'
        ]
        self.diversification_threshold = diversification_threshold

    def evaluate(self, agent_scores):
        if not agent_scores:
            raise ValueError("No agent scores")
    ...
    def check_diversification(self, s):
        ...
        for dim in DIMENSIONS:
            val = getattr(s, dim, 0.0)
            if val < t:
                g.append(f"{dim} below threshold ({t})")
        return g
...
def score_from_dict(d):
    if d is None or not isinstance(d, dict):
        raise TypeError(f"Expected dict or ScoreCard, got {type(d).__name__ if d is not None else 'None'}")
    s = ScoreCard()
    for dim in DIMENSIONS:
