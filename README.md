...
Mastery Gate: diversified non-local per PRD area
...
→ Auto Skill Update
→ Mastery Gate (diversified non-local checks)
→ Output: full PRD document + updated skill

Phase 1-3: iterate on code
→ 33 agents per phase, 11 per point
→ Mastery Gate between phases
→ Auto Skill Update after each phase
